              ...
              {chapters.map((chapter) => (
                <article key={chapter.city} className="overflow-hidden rounded-[2.25rem] border border-asha-line bg-white p-5 shadow-soft transition hover:-translate-y-1 hover:shadow-lift">
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-7">
                      {chapter.instagram ? <a href={chapter.instagram} target="_blank" rel="noreferrer" className="group inline-flex items-center gap-3 rounded-full bg-[linear-gradient(135deg,#1F5EFF_0%,#1877FF_52%,#00AEF0_100%)] px-6 py-3 text-sm font-black text-white shadow-[0_14px_30px_rgba(31,94,255,.25)] transition hover:-translate-y-1 hover:scale-[1.03] hover:shadow-[0_18px_38px_rgba(31,94,255,.4)]"><span className="grid h-7 w-7 place-items-center rounded-full bg-white/20"><Instagram size={16} /></span><span>Follow on Instagram</span><span className="text-white/70 transition group-hover:translate-x-1">↗</span></a> : <CTAButton href="/get-involved" tone="coral">Join this chapter</CTAButton>}
                    </div>
                  </div>
                </article>
              ))}
